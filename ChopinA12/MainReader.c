 ...
 * -------------------------------------------------------------
 */
chopin_void bErrorPrint(chopin_string fmt, ...);
chopin_void displayBuffer(Buffer* ptr_Buffer);
chopin_long getFileSize(chopin_string fname);
chopin_intg isNumber(const chopin_string ns);
chopin_void startReader(chopin_string, chopin_string, chopin_char, chopin_intg, chopin_intg);
...
chopin_intg mainReader(chopin_intg argc, chopin_string* argv) {
	...
	chopin_string program = argv[0];
	chopin_string input = argv[2];
	chopin_char mode = MODE_FIXED;
	chopin_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
chopin_void startReader(chopin_string program, chopin_string input, chopin_char mode, chopin_intg size, chopin_intg increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	chopin_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	chopin_char symbol;			/* symbol read from input file */
