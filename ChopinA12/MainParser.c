...
chopin_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern chopin_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern chopin_intg line; /* source code line number - defined in scanner.c */
...
extern chopin_void startParser(chopin_void);
extern chopin_intg startScanner(BufferPointer sc_buf);

static chopin_void printParserError(chopin_string fmt, ...);
static chopin_void displayParser(BufferPointer ptrBuffer);
static chopin_long getParserFileSize(chopin_string fname);
static chopin_void callGarbageCollector(chopin_void);
...
chopin_intg mainParser(chopin_intg argc, chopin_string* argv) {
	...
	chopin_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
chopin_void printParserError(chopin_string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(chopin_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
chopin_long getParserFileSize(chopin_string fname) {
	FILE* input;
	chopin_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
chopin_void displayParser(BufferPointer ptrBuffer) {
...
chopin_void callGarbageCollector(chopin_void) {
